# stock_analysis_pytdx_production.py - 最终 pytdx (通达信) 稳定版 (日期容错处理与空数据帧检查)
...
def aggregate_and_analyze(df_raw_slice, freq, prefix):
    ...
    # 注意：这里的 index 应该是 date 对象，但为了 resample 需要转为 datetime
...
def get_full_history_data(api, market, code, freq):
    """
    使用 pytdx 分页获取完整的历史 K 线数据，并增加日期解析容错。
    """
    all_data = []
    ...
    if all_data:
        ...
        # 【修正点 2】：安全检查：如果数据帧变空，则安全返回空 DataFrame，避免 .dt 错误
        if df_combined.empty:
             logger.warning(f"    - {code} 移除无效日期后数据为空。")
             return pd.DataFrame()

        # 3. 提取有效的日期部分 (此时 'valid_datetime' 保证是 datetime 类型)
...
def get_and_analyze_data_slice(api, market, code, start_date):
    ...
    try:
        ...
        # 注意：这里 df_full 的 index 已经是 date 对象
